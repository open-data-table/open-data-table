...
import { OpenDataTableController } from '../src/open-data-table-controller.js';
...
class TestDataTableController extends OpenDataTableController(LitElement) {
    
}

window.customElements.define('test-data-table-controller', TestDataTableController);

describe('open-data-table-controller', () => {
    ...
    it('fires a selection changed event', async () => {  
        ...
        let e = await oneEvent(el, 'open-data-table-selection-changed');
        await nextFrame();
        expect(e).to.not.equal(null);

        setTimeout(() => el.deselectRow(1));
        e = await oneEvent(el, 'open-data-table-selection-changed');
        await nextFrame();
        expect(e).to.not.equal(null);
    ...
    it('allows columns to be set by property', async () => {  
        const el = await fixture(html`
            <test-data-table-controller .columns="${columns}" .rows="${rows}">
            </test-data-table-controller>
        `);
        ...
        el.columns = columns;
        expect(el.columns).to.not.equal(null);
    });

    it('does not allow read only properties to be set', async () => {  
        const el = await fixture(html`
            <test-data-table-controller .columns="${columns}" .rows="${rows}">
            </test-data-table-controller>
        `);
